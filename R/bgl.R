...
betas.post = function(betas.p, p, Sigma.b, Y, Y0,X, W, betas, delta, C, rho, form) {
  ...
  lpost = llikWeibull2(Y,Y0, eXB, delta, C, rho) + lprior
  return(lpost)
}
...
gammas.post = function(gammas.p, p, Sigma.g, Y, eXB, Z, gammas, C, rho, form) {
  ...
  lpost = llikWeibull2(Y, eXB, delta, C, rho) + lprior
  return(lpost)
}
...
mcmcSpatialCure2 <- function(Y, C, X, Z, S, A, N, burn, thin, w = c(1, 1, 1), m = 10, form, prop.var) {
  ...
  for (iter in 1:N) {
    ...
    V = V.MH.sampling(S, A, lambda, Y, eXB, Z, V, gammas, C, rho, prop.var)
 	gammas = gammas.slice.sampling_2(Sigma.g, Y, eXB, Z, V, gammas, C, rho, w[2], m, form = form)
    delta = 1 / (1 + exp(- Z %*% gammas- V))
